...
if (isset($id_jenis_bayar) || isset($harga) || isset($transaksi_kendaraan) || isset($tgl_pinjam) || isset($tgl_kembali) || isset($jenis_bayar)) {
    ...
    $denda = $_POST['denda'];   
            ...
            //melakukan proses pembuatan query update
            $sql="UPDATE `tbl_jenis_bayar` SET `id_jenis_bayar`='?',`jenis_bayar`='?',`transaksi_kendaraan`='?',`tgl_pinjam`='?',`tgl_kembali`='?',`harga`='?',`denda`='?' WHERE id_jenis_bayar";
            $tbl_jenis_bayar = mysqli_query($koneksi,"SELCT * FROM tbl_jenis_bayar WHERE id_jenis_bayar");

            //periksa, apakah query update itu sudah berhasil atau belum
            if ($query()) {
