                        ...
                        <div class="col-sm-12">
                            <div class="clearfix">
                                ...
                                <div class="col card-header text-left">
                                    <h1 class="h1 mb-5 font-weight-bold text-gray-900">List Vehicle</h1>
                                </div>
                                <div class="col card-header text-right">
                                    <a href="tambahkendaraan.php" class="btn btn-success float-end"><i class="fa fa-plus"></i>&nbsp;Add</a>
                                    </div>
                                </div>
                                <div class="card-body">
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                            <?php
                                            //preparing showing data
                                            include("koneksi.php");
                                            $sql="SELECT * FROM tblkendaraan";
                                            $no=1;
                                            $tblkendaraan = mysqli_query($koneksi,"SELECT * FROM tblkendaraan ORDER BY id_kendaraan ASC");
                                            while ($data = mysqli_fetch_array($tblkendaraan)) :
                                            ?>

                                            <tr>
                                                <td><?= $no++; ?></td>
                                                <td><?= $data['merk']; ?></td>
                                                <td><img src="../../assets/image/" alt="foto" class="img-fluid" style="width: 300px;"><?= $data['foto']; ?></td>
                                                <td><?= $data['no_polisi']; ?></td>
                                                <td><?= $data['warna']; ?></td>
                                                <td><?= $data['tahun']; ?></td>
                                                <td><?= $data['keterangan']; ?></td>
                                                <td><?= $data['status_kendaraan']; ?></td>
                                                <td><?= $data['tarif_kendaraan']; ?></td>
                                                <td><?= $data['harga']; ?></td>
                                                <td><?= $data['denda']; ?></td>
                                                <td>
                                                    <a href="ubahkendaraan.php?id=<?= $data['id_kendaraan']; ?>" class="btn btn-primary"><i class="fa fa-edit"></i>&nbsp;Change</a>
                                                    <a href="hapuskendaraan.php?id=<?= $data['id_kendaraan']; ?>&gambar=<?= $data['foto']; ?>" class="btn btn-danger" onClick="return confirm('Are you sure want to delete this data!')"><i class="fa fa-trash"></i>&nbsp;Delete</a>
                                                </td>
                                            </tr>
                                       <?php endwhile; ?>
                                    </tbody>
